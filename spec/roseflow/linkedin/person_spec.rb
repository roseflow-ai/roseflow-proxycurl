      ...
      describe "#find" do
        ...
        it "returns nil with an invalid URL" do
          VCR.use_cassette("linkedin/person/find_invalid", record: :new_episodes) do
            object = described_class.new.find("https://www.linkedin.com/in/foo-bar-does-not-exist/")
      ...
      describe "#lookup" do
        ...
        it "returns a person with valid parameters" do
          VCR.use_cassette("linkedin/person/lookup", record: :new_episodes) do
            expect(described_class.new.lookup(domain: company_domain, first_name: first_name)).to eq "https://fi.linkedin.com/in/anssi-p-k%C3%A4rkk%C3%A4inen"
          end
        end

        it "returns nil when it can't find a person" do
          VCR.use_cassette("linkedin/person/lookup_invalid", record: :new_episodes) do
            expect(described_class.new.lookup(domain: company_domain, first_name: "Nalle", last_name: "Puh")).to be_nil
          end
        end
      end
      ...
      describe "#role" do
        ...
        it "returns a person" do
          VCR.use_cassette("linkedin/person/role", record: :new_episodes) do
            expect(described_class.new.role(company_name: company_name, role: role)).to be_a Person::Object
          end
        end
      end
    end
